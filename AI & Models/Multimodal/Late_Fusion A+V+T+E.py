...
## Visual (Image/Video based and Micro Expressions) models
...
data_path = "../Expressions/Feature Vectors Court Trial"

X_train_expressions, X_test_expressions, y_train_expressions, y_test_expressions = ppe.get_manual_split_data(data_path, manual_split_video_names_path)
...
# Get predictions for the expressions data
predictions_train_expressions = expressions_model.predict_proba(X_train_expressions)[:,1].ravel()
predictions_test_expressions = expressions_model.predict_proba(X_test_expressions)[:,1].ravel()
...
predictions_train_expressions_thresholded = (predictions_train_expressions > threshold).astype(int)
predictions_test_expressions_thresholded = (predictions_test_expressions > threshold).astype(int)
...
majority_vote_train = mode(np.c_[predictions_train_video_thresholded, predictions_train_video_thresholded, predictions_train_expressions_thresholded, predictions_train_audio_thresholded, predictions_train_text_thresholded], axis=1)[0].flatten()

# Majority voting on test data
majority_vote_test = mode(np.c_[predictions_test_video_thresholded, predictions_test_video_thresholded, predictions_test_expressions_thresholded, predictions_test_audio_thresholded, predictions_test_text_thresholded], axis=1)[0].flatten()
...
weighted_vote_train = np.average(np.c_[predictions_train_video, predictions_train_expressions, predictions_train_audio, predictions_train_text], axis=1, weights=weights)
weighted_vote_train = (weighted_vote_train > threshold).astype(int)

# Weighted voting on test data
weighted_vote_test = np.average(np.c_[predictions_test_video, predictions_test_expressions, predictions_test_audio, predictions_test_text], axis=1, weights=weights)
...
X_train_concat = np.c_[predictions_train_video,
                       predictions_train_expressions,
                       predictions_train_audio, 
                       predictions_train_text,]

X_test_concat = np.c_[predictions_test_video, 
                      predictions_test_expressions,
